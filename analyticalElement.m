...
time = tic;
if (buildPhi)
    %% Compute objective functional - Average Temperature over Domain
    fprintf('Computing phi (Average Temperature)... \n');
    % Average temperature at element level: (T1+T2+T3+T4)/4 * element_area
    % For Lx=1.0, Ly=1.0 domain, each element contributes to total average
    % Note: The integration will square dx and dy, so we use the original temperature interpolation
    phi = (T1 + T2 + T3 + T4)/4;
...
if (exportPhi)
    ...
    phiVars = {dx,dy,T1,T2,T3,T4};
    f = matlabFunction(phi,'File','PHI','Comments','Version: 1.0 - Average Temperature Objective','Vars',phiVars,'Optimize',false);
    fprintf('Exporting dphi/dg... \n');
    dphidgVars = {dx,dy,alpha,dalpha,T1,T2,T3,T4};
    f = matlabFunction(dphidg,'File','dPHIdg','Comments','Version: 1.0 - Average Temperature Objective','Vars',dphidgVars,'Optimize',false);
    fprintf('Exporting dphi/ds... \n');
    dphidsVars = {dx,dy,T1,T2,T3,T4};
    f = matlabFunction(dphids,'File','dPHIds','Comments','Version: 1.0 - Average Temperature Objective','Vars',dphidsVars,'Optimize',false);
end
if (exportJR)
